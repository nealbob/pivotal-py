...
class TestSemanticValidation:
    ...
    def test_missing_table_caught(self, parser, ns):
        errors = self._validate(parser, 'with slaes as summary', ns)
        assert len(errors) == 1
    ...
    def test_valid_table_no_error(self, parser, ns):
        errors = self._validate(parser, 'with sales as summary\n    filter revenue > 100', ns)
        assert errors == []
    ...
    def test_missing_concat_table(self, parser, ns):
        errors = self._validate(parser, 'with combined\n    concat sales, missing_table', ns)
    ...
    def test_within_cell_load_then_use(self, parser, ns):
        """load then with in same cell — no false positive."""
        code = 'load "data.csv" as sales\nwith sales as summary\n    sort revenue desc'
        ast = parser.parse(code)
    ...
    def test_within_cell_copy_then_use(self, parser, ns):
        """with X as Y then with Y as Z — no false positive."""
        code = 'with sales as summary\n    group by region\n        agg sum revenue as total\nwith summary as result\n    sort total desc'
